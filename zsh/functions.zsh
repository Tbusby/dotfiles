...
function gifit () {
    re='^[0-9]+$'
    mkdir -p ./tmp_pngs
    ffmpeg -i $1 -r 10 ./tmp_pngs/out%04d.png
    mkdir -p ./tmp_gifs
    sips -s format gif ./tmp_pngs/*.png --out ./tmp_gifs
    cd ./tmp_gifs
    if ! [[ -z "$2" ]]
    then
        gifsicle *.gif --optimize=3 --delay=3 --loopcount --colors=256 --scale=$2 > ../${1%.*}.gif
    else
        gifsicle *.gif --optimize=3 --delay=3 --loopcount --colors=256 --scale=1 > ../${1%.*}.gif
    fi
    cd ..
}
